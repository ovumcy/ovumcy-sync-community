...
func TestParseCoverageProfileAndLineState(t *testing.T) {
	profile := "mode: atomic\n" +
		"github.com/ovumcy/ovumcy-sync-community/internal/db/sqlite.go:12.45,17.2 3 1\n" +
		"github.com/ovumcy/ovumcy-sync-community/internal/db/sqlite.go:20.2,22.3 2 0\n"

	cov := parseCoverageProfile(profile, "github.com/ovumcy/ovumcy-sync-community")
...
func TestIsGatedFile(t *testing.T) {
	for _, f := range []string{"internal/db/sqlite.go", "cmd/ovumcy-sync-community/main.go"} {
		if !isGatedFile(f) {
...
func TestReadModulePath(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte("module github.com/ovumcy/ovumcy-sync-community\n\ngo 1.26.4\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if got != "github.com/ovumcy/ovumcy-sync-community" {
