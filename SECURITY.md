...
| A blob generation within 2^32 of the int64 ceiling is rejected, so a crafted maximal write cannot strand an owner's blob at an unbeatable generation | `TestPutBlobRejectsGenerationNearInt64Ceiling` in [internal/services/sync_service_blob_generation_test.go](internal/services/sync_service_blob_generation_test.go) |
